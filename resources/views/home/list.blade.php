...
@section('content')
    <article class="blogs">
        <h1 class="t_nav">
            ...
            <a href="{{url('cate/'.$field->id)}}" class="n2">{{$field->cate_name}}</a>
        </h1>
        <div class="newblog left">
            @foreach($data as $d)
            <h2>{{$d->art_title}}</h2>
            <p class="dateview">&nbsp;<span>发布时间：{{date('Y-m-d', $d->art_time)}}</span><span>作者：{{$d->art_editor}}</span><span>分类：[<a
                            href="{{url('cate/'.$field->id)}}">{{$field->cate_name}}</a>]</span></p>
                ...
                <div class="rnav">
                    <ul>
                        @foreach($submenu as $k=>$s)
                            <li class="rnav{{$k+1}}">
                                <a href="{{url('cate/'. $s->id)}}" target="_blank">{{$s->cate_name}}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
